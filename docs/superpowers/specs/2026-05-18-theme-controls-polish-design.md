...
在现有主题系统基础上，补一轮控件视觉打磨，重点解决三个已确认问题：

- 下拉框仍然像原生默认控件，和品牌主题脱节
- `PlayerWindow` 底部播放控制按钮层级不清，部分图标在深色控制区里可见性不足
- `PlayerWindow` 的进度条和音量条状态反馈偏弱，和新的按钮层级不一致
...
- 强化进度条和音量条的品牌色填充、hover、pressed、滑块可见性
- 解决深色控制区里黑色图标不清晰的问题
...
- 已播放段使用品牌橙填充
- `hover` 时滑块放大并提亮边框
- `pressed` 时滑块实心化，拖拽感更明确
- 进度条比音量条更粗、更强调；音量条沿用同一视觉语言但保持更紧凑
