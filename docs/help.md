...
- **预加载下一集**：播放中提前解析下一集播放地址并预热网盘直链代理，自动连播时更快起播；失败会静默回退到原有流程
- **播放缓存大小**：`16` - `4096 MB`
...
- **播放遥测徽章**：底部控制条实时显示实际解码分辨率、编码器、硬解状态、帧率、码率、下行速率、缓冲与丢帧（1 秒刷新一次，纯信息展示，不影响播放性能）；可在播放器右键菜单「播放遥测」开关并记住选择。
- **用户自定义 `mpv` 配置与着色器**：约定目录下放入 `mpv.conf`、`shaders/` 即生效，详见 14.1 / 14.2。
...
### 14.1 用户自定义 `mpv` 配置（`mpv.conf`）

约定与自定义 `libmpv` 相同，应用启动时依次在以下目录寻找 `mpv.conf`，找到即启用（先者胜）：

1. 用户目录下的 `mpv` 目录，例如 `~/mpv/mpv.conf`（Windows 为 `C:\Users\<用户名>\mpv\mpv.conf`）
2. 应用目录下的 `lib` 子目录，例如 `<应用目录>/lib/mpv.conf`
3. 应用目录本身，例如 `<应用目录>/mpv.conf`

说明：

- 命中的目录同时成为 `mpv` 的配置目录：放在同处的 `input.conf`（自定义按键）、`scripts/` 下的 Lua 脚本等存在即生效
- 应用自身的关键选项（窗口嵌入、视频输出、硬解、缓存策略等）优先级高于 `mpv.conf`，用户配置不会破坏嵌入渲染；高级设置里的「更多 MPV 配置」优先级最高
- 未找到 `mpv.conf` 时行为与旧版本完全一致；是否生效可按 `F1` 在系统信息里查看「mpv 配置」一行

### 14.2 着色器预设（Anime4K / FSRCNNX 等）

在上述约定目录下放置 `shaders/` 目录即可被识别：

- `shaders/<预设名>/*.glsl`：每个子目录是一个预设，例如把 Anime4K 按 `Mode_A_High`、`Mode_B_Low` 等分组放入不同子目录
- `shaders/*.glsl`：直接散放在 `shaders/` 根下的着色器归入「默认」预设
- 播放器右键菜单「着色器」中可运行时切换或关闭预设，即时生效，选择会被记住并在下次播放时自动应用
- 支持的文件后缀为 `.glsl` 和 `.hook`；着色器按文件名排序后依序加载
