...
def build_secspider_package(
    ...
    *,
    ...
    package_id: str | None | object = _AUTO_PACKAGE_ID,
    ...
    wrap_key = HKDF(
        ...
        hashmod=SHA256,
        num_keys=1,
        context=f"secspider:{name}:{version}:wrap-key".encode(),
    )
    wrap_nonce = HKDF(
        master=master_secret,
        key_len=12,
        ...
        hashmod=SHA256,
        num_keys=1,
        context=f"secspider:{name}:{version}:wrap-nonce".encode(),
    )
    ...
    if package_id is _AUTO_PACKAGE_ID:
        headers["id"] = generate_secspider_id(name)
    elif package_id is not None:
        headers["id"] = package_id
    signing_bytes = "\n".join(
        [
            f"//@{key}:{headers[key]}"
            for key in (
    ...
    signature = eddsa.new(signing_key, "rfc8032").sign(signing_bytes)
    headers["sig"] = "base64:" + b64encode(signature).decode("ascii")
    package_text = serialize_package_text(headers, payload_b64)
    keyring = StaticSpiderKeyring(
        public_keys={kid: public_key},
        master_secrets={kid: master_secret},
    )
